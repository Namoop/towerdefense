...
function Game() {
	...
	this.onEvent = {};
	this.mouse = {
		get x() { return (me.windowMouseX - me.cnv.getBoundingClientRect().left) /me.scaleX },
		get y() { return (me.windowMouseY - me.cnv.getBoundingClientRect().top) /me.scaleY },
	...
	this.setup = function(scale, scaleTo) {
		...
		onkeydown = onkeyup = function (e) {
			...
			try { me.onEvent[event](event, Date.now()-_kp[e.key]) } catch(e){}
		...
		onmousedown = function (e) {
			...
			if (!e.button) me.onleftmousedown?.(e)
			else if (e.button == 2) me.onrightmousedown?.(e)
		}
	}
	this.loop = function () {
		if (me.clearEveryFrame) me.draw.clear();
		me.run?.()
		if (!me.stop) window.requestAnimationFrame(me.loop)
	}
	this.draw = {
		...
		ellipse(x, y, radius1, radius2, fill) {
			me.pen.beginPath();
			me.pen.ellipse(x*me.scaleX, y*me.scaleY, radius1*me.scaleX, radius2*me.scaleY, 0, 0, Math.PI * 2);
			if (fill) me.pen.fill();
			else me.pen.stroke();
		},
